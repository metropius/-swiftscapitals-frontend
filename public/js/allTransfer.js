(function () {
  ...
  function renderTransfersTable(transfers, page) {
    ...
    const rows = transfers
      .map((t, index) => {
        ...
        const type = escapeHtml(t.type || 'N/A');
        const amount = escapeHtml(t.amount != null ? String(t.amount) : '0.00');
        const from = escapeHtml(String(t.transferFrom || 'usd').toUpperCase());
        ...
        const ownerName = t.owner
          ? escapeHtml(
              `${t.owner.firstname || ''} ${t.owner.lastname || ''}`.replace(/\s+/g, ' ').trim() || '—'
            )
          : '—';
        const dateStr = t.createdAt
          ? escapeHtml(new Date(t.createdAt).toLocaleDateString())
          : '—';
        const id = escapeHtml(t._id);
        ...
        return `
          <tr class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">${from}</td>
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
              <div class="flex items-center space-x-4">
                <a href="viewTransfer.html?id=${id}" class="text-blue-600 hover:text-blue-800" title="View">
                  <i data-lucide="eye" class="w-5 h-5"></i>
                </a>
                <a href="editTransfer.html?id=${id}" class="text-amber-600 hover:text-amber-800" title="Edit">
                  <i data-lucide="edit-2" class="w-5 h-5"></i>
                </a>
                <button
                  type="button"
                  data-id="${id}"
                  class="text-red-600 hover:text-red-800 delete-transfer-btn"
                  title="Delete">
    ...
    document.querySelectorAll('.delete-transfer-btn').forEach((btn) => {
      btn.onclick = async () => {
        ...
        try {
          ...
          const res = await api.delete(`/deleteTransfer/${id}`);
          const data = res.data;
          ...
          await Swal.fire({
            icon: 'success',
            title: 'Deleted',
            text: data.message || 'Transfer deleted successfully',
            timer: 2000,
          ...
          Swal.fire({
            icon: 'error',
            title: 'Error',
            text: err.response?.data?.message || err.message || 'Failed to delete transfer',
  ...
  async function loadTransfers() {
    try {
      ...
      const res = await api.get('/allTransfer', {
      ...
      renderTransfersTable(transfers, page);
      updatePagination(page, total);
    } catch (err) {
      console.error('All transfers load error:', err);
        ...
        tbody.innerHTML = `
          <tr>
            <td colspan="8" class="px-6 py-12 text-center text-red-500">
              ${escapeHtml(err.response?.data?.message || err.message || 'Failed to load transfers')}
            </td>
          </tr>
        `;
      }
      ...
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: err.response?.data?.message || err.message || 'Failed to load transfers',
